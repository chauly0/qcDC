...
% Code updated by Kendra Nyberg (February 2017)

% Inputs
%   - videoSearchPath: specifies the default directory to select videos
%   - ext: string that specifies video file extension (e.g. '.cine')
...
%   Adds input variable for file extension preference (.avi or .cine)

function [pathNames, videoNames] = PromptForVideos(videoSearchPath, ext)
...
if(~DEBUG_FLAG)
    ...
    while (lastFilename ~= 0)
        [fileNames, filePath] = uigetfile(ext, 'Please select the next video file, if done select cancel.', filePath, 'multiselect', 'on');
        lastFilename = 0;
		if(iscell(fileNames))
    ...
    if ~exist('searchPaths', 'var')
        ...
        for i = 1:length(searchPaths)
            ...
            allVids = dir(fullfile(pathToSearch, ['*' ext]));
